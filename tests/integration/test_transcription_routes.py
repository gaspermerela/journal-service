...
@pytest.mark.asyncio
async def test_auto_promote_first_transcription(authenticated_client, sample_voice_entry, mock_transcription_service, db_session):
    ...
    # Check that transcription was auto-promoted to primary
    transcription = await db_service.get_transcription_by_id(db_session, transcription_id)
    assert transcription is not None
    assert transcription.status == "completed"
    assert transcription.is_primary is True

    # Verify it's the primary transcription for the entry
    primary = await db_service.get_primary_transcription(db_session, sample_voice_entry.id)
    assert primary is not None
    assert primary.id == transcription_id


@pytest.mark.asyncio
async def test_second_transcription_not_auto_promoted(authenticated_client, sample_voice_entry, mock_transcription_service, db_session):
    ...
    # Create first transcription and set as primary
    trans1_data = TranscriptionCreate(
        entry_id=sample_voice_entry.id,
        status="completed",
        model_used="whisper-base",
    ...
    assert response.status_code == 202
    trans2_id = response.json()["transcription_id"]

    # Wait for background task to complete
    await asyncio.sleep(0.3)

    # Check that second transcription was NOT auto-promoted
    trans2 = await db_service.get_transcription_by_id(db_session, trans2_id)
    assert trans2 is not None
    assert trans2.status == "completed"
    assert trans2.is_primary is False  # Should NOT be primary

    # Verify first transcription is still primary
    primary = await db_service.get_primary_transcription(db_session, sample_voice_entry.id)
    assert primary is not None
    assert primary.id == trans1.id  # First transcription should still be primary
