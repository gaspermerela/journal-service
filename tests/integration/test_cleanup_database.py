...
class TestCleanedEntryDatabaseParameters:
    ...
    async def test_create_cleaned_entry_with_temperature_top_p(
        ...
        cleaned_entry = CleanedEntry(
            ...
            analysis={"themes": ["test"], "emotions": [], "characters": [], "locations": []},
    ...
    async def test_update_cleaned_entry_parameters(
        ...
        """Test updating cleaned entry parameters via update_cleaned_entry_processing."""
        # Create initial cleaned entry without parameters
        cleaned_entry = CleanedEntry(
            id=uuid.uuid4(),
            voice_entry_id=sample_voice_entry.id,
            transcription_id=sample_transcription.id,
        ...
        updated_entry = await db_service.update_cleaned_entry_processing(
            ...
            analysis={"themes": ["updated"], "emotions": [], "characters": [], "locations": []},
            temperature=0.8,
            top_p=0.9
        )

        # Verify parameters were updated
        assert updated_entry.temperature == 0.8
        assert updated_entry.top_p == 0.9
        assert updated_entry.cleaned_text == "Updated cleaned text"
