...
Quality speech-to-text for smaller languages like Slovenian remains a challenge. Generic multilingual models often struggle with accuracy, while specialized solutions can be expensive or difficult to integrate. For professionals handling sensitive recordings—therapists, healthcare workers, legal practitioners—there's an additional barrier: GDPR compliance when using cloud-based services.

## How This Started

I wanted to record dream journals by voice instead of typing at 3am. That simple problem led me down a rabbit hole: transcription, LLM-based text cleanup, quality testing across providers, and eventually realizing that therapists and healthcare professionals face the same pain—but with much higher stakes.
...
4. **Analyzes** content (extracts themes, emotions, key points—useful for session summaries)
